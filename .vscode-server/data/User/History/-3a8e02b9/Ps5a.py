...
class LoadBalancer(app_manager.RyuApp):
    ...
    def send_proxied_arp_response(self, src_mac, src_ip, msg):
    ...
    def packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if src_ip in self.server_to_client.keys():
                self.send_proxied_arp_response(dp=datapath, src_mac=src_mac, src_ip=src_ip)
            ...
            if src_ip in self.server_to_client.keys():
                self.send_ip_response(dp=datapath, src_mac=src_mac, src_ip=src_ip)
            else:
